...
if __name__ == "__main__":
    ...
    with open("./dataset/" + args.dataset, "r") as fin:
        with open(f"./output/{args.model_name}.{args.dataset}", "w") as fout:
            datas = []
            for line in fin:
                line = line.strip()
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=args.concurrent) as executor:
                futures = []
                for data in datas:
                    if args.dataset not in ["factscore_testset.jsonl", "test_data.jsonl"]:
